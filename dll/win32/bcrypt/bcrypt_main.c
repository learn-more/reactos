...
#if defined(HAVE_GNUTLS_CIPHER_INIT) || defined(HAVE_COMMONCRYPTO_COMMONCRYPTOR_H) && MAC_OS_X_VERSION_MAX_ALLOWED >= 1080 || defined(__REACTOS__)
                             ...
                             ULONG iv_len, UCHAR *output, ULONG output_len, ULONG *ret_len, ULONG flags )
{
    ...
    if (key->u.s.mode == MODE_ID_GCM)
    {
        ...
        *ret_len = input_len;
        ...
        if ((status = key_symmetric_set_auth_data( key, auth_info->pbAuthData, auth_info->cbAuthData )))
            return status;
#ifdef __REACTOS__
        if ((status = key_symmetric_decrypt( key, input, input_len, output, output_len, tag, sizeof(tag)))) return status;
#else
    ...
    *ret_len = input_len;
    ...
    while (bytes_left >= key->u.s.block_size)
    {
        ...
        if ((status = key_symmetric_decrypt( key, src, key->u.s.block_size, dst, key->u.s.block_size, tag, sizeof(tag))))
            return status;
#else
    ...
    if (flags & BCRYPT_BLOCK_PADDING)
    {
        ...
        if (!(buf = heap_alloc( key->u.s.block_size ))) return STATUS_NO_MEMORY;
#ifdef __REACTOS__
        status = key_symmetric_decrypt( key, src, key->u.s.block_size, buf, key->u.s.block_size, tag, sizeof(tag));
#else
